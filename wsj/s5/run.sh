...
if [ $stage -le 0 ]; then
  ...
  for x in test_eval92 test_eval92_5k test_eval93 test_eval93_5k test_dev93 test_dev93_5k train_si284 train_si84 dev_dt_05 dev_dt_20; do
  ...
  utils/combine_data.sh data/train data/test_eval92 data/test_eval92_5k data/test_eval93 data/test_eval93_5k data/test_dev93 data/test_dev93_5k data/train_si284 data/train_si84 data/dev_dt_05 data/dev_dt_20 || exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  if $train; then
    ...
    steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" 2000 10000 \
      data/train data/lang_nosp exp/mono_ali exp/tri1 || exit 1;
  fi
...
if [ $stage -le 3 ]; then
  ...
  if $train; then
    ...
    steps/train_lda_mllt.sh --cmd "$train_cmd" \
      ...
      data/train data/lang_nosp exp/tri1_ali exp/tri2 || exit 1;
  fi
...
if [ $stage -le 4 ]; then
  ...
  if $train; then
    steps/align_si.sh  --nj $nj --cmd "$train_cmd" \
      data/train data/lang_nosp exp/tri2 exp/tri2b_ali  || exit 1;

    steps/train_sat.sh --cmd "$train_cmd" 4200 40000 \
      data/train data/lang_nosp exp/tri2b_ali exp/tri3 || exit 1;
  fi
...
if [ $stage -le 5 ]; then
  ...
  steps/get_prons.sh --cmd "$train_cmd" \
    data/train data/lang_nosp exp/tri3 || exit 1;
  utils/dict_dir_add_pronprobs.sh --max-normalize true \
    data/local/dict_nosp \
    exp/tri3/pron_counts_nowb.txt exp/tri3/sil_counts_nowb.txt \
    exp/tri3/pron_bigram_counts_nowb.txt data/local/dict || exit 1
  ...
  utils/dict_dir_add_pronprobs.sh --max-normalize true \
    ...
    exp/tri3/pron_counts_nowb.txt exp/tri3/sil_counts_nowb.txt \
    exp/tri3/pron_bigram_counts_nowb.txt data/local/dict_larger || exit 1
...
if [ $stage -le 6 ]; then
  ...
  if $train; then
    steps/train_sat.sh  --cmd "$train_cmd" 4200 40000 \
      data/train data/lang exp/tri3 exp/tri4 || exit 1;
  fi
