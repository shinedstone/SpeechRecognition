...
class ServerWebsocket(WebSocketClient):
    ...
    def _on_result(self, result, final):
        try:
            ...
            processed_transcripts = result.replace('<UNK>', '')
            if processed_transcripts:
                logger.info("%s: Postprocessing done." % self.request_id)
                event = dict(status=common.STATUS_SUCCESS,
                             segment=self.num_segments,
                             result=dict(hypotheses=[dict(transcript=processed_transcripts)], final=final))
                try:
                    self.send(json.dumps(event))
                except:
                    e = sys.exc_info()[1]
    ...
    def _on_full_result(self, full_result_json):
        try:
            ...
            if full_result.get("status", -1) == common.STATUS_SUCCESS:
                ...
                full_result = self.post_process_json(full_result)
                logger.info("%s: Postprocessing done." % self.request_id)
    ...
    def _on_eos(self, data=None):
        ...
        while self.num_processing_threads > 0:
            logger.debug("Waiting until processing threads finish (%d)" % self.num_processing_threads)
        ...
        if hasattr(self.decoder_pipeline, 'get_adaptation_state'):
            ...
            event = dict(status=common.STATUS_SUCCESS,
                         adaptation_state=dict(id=self.request_id,
                                               value=base64.b64encode(zlib.compress(adaptation_state.encode("utf-8"))).decode("utf-8"),
    ...
    def post_process(self, texts, blocking=False):
        if self.post_processor:
            logger.debug("%s: Waiting for postprocessor lock" % self.request_id)
            if blocking:
                timeout=None
            else:
                timeout=0.0
            try:
                ...
                logger.debug("%s: Skipping postprocessing since post-processor already in use"  % (self.request_id))
    ...
    def post_process_json(self, event):
        try:
            if "result" in event:
                if len(event["result"]["hypotheses"]) > 1:
                    ...
                    confidence = 1 - math.exp(-confidence)
                else:
                    confidence = 1.0e+10;
                event["result"]["hypotheses"][0]["confidence"] = confidence

                trans = re.sub(r'\d{5,}', '', event["result"]["hypotheses"][0]["transcript"].replace('<UNK>', '').replace(' ', ''))
                if len(trans) <= 1:
                    event["result"]["hypotheses"][0]["transcript"] = "\n"
                else:
                    event["result"]["hypotheses"][0]["transcript"] = spacing(trans) + "\n"
...
def main():
    ...
    logging.basicConfig(filename="log/worker_" + datetime.now().strftime('%Y-%m-%d_%H:%M:%S') + "_" + str(os.getpid()) + ".log" , filemode='a', format='%(asctime)s:%(msecs)d %(name)s %(levelname)s %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=logging.DEBUG)
    ...
    if args.fork > 1:
        logger.info("Forking into %d processes" % args.fork)
    ...
    _thread.start_new_thread(GObject.MainLoop().run, ())
