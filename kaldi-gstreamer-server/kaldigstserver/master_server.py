...
class HttpChunkedRecognizeHandler(tornado.web.RequestHandler):
    ...
    def prepare(self):
        ...
        logger.info("%s: OPEN: user='%s', content='%s'" % (self.id, self.user_id, self.content_id))
        self.worker = None
        self.error_status = 0
        ...
        try:
            ...
            logger.warn("%s: No worker available for client request" % self.id)
            self.set_status(503)
            self.finish("No workers available")

    def data_received(self, chunk):
        assert self.worker is not None
        logger.debug("%s: Forwarding client message of length %d to worker" % (self.id, len(chunk)))
    ...
    def end_request(self, *args, **kwargs):
        ...
        if self.error_status == 0:
            ...
            logger.info("%s: Error (status=%d) processing HTTP request: %s" % (self.id, self.error_status, self.error_message))
    ...
    def send_event(self, event):
        ...
        logger.info("%s: Receiving event %s from worker" % (self.id, event_str))
        if event["status"] == 0 and ("result" in event):
            try:
                ...
                e = sys.exc_info()[0]
                logger.warn("Failed to extract hypothesis from recognition result:" + e)
        elif event["status"] != 0:
...
class ReferenceHandler(tornado.web.RequestHandler):
    def post(self, *args, **kwargs):
        ...
        if content_id:
            ...
            logger.info("Received reference text for content %s and user %s" % (content_id, user_id))
...
class WorkerSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        logger.info("New worker available " + self.__str__())
        self.application.send_status_update()
...
class DecoderSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        logger.info("%s: Request arguments: %s" % (self.id, " ".join(["%s=\"%s\"" % (a, self.get_argument(a)) for a in self.request.arguments])))
    ...
    def on_message(self, message):
        assert self.worker is not None
        logger.info("%s: Forwarding client message (%s) of length %d to worker" % (self.id, type(message), len(message)))
...
def main():
    ...
    logging.basicConfig(filename="log/master_server_" + datetime.now().strftime('%Y-%m-%d_%H:%M:%S') + "_" + str(os.getpid()) + ".log" , filemode='a', format='%(asctime)s:%(msecs)d %(name)s %(levelname)s %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=logging.DEBUG)
    ...
    if options.certfile and options.keyfile:
        ...
        logger.info("Using SSL for serving requests")
