...
chain_train=true
decode=false # set false if you don't want to decode each GMM model
...
if [ $stage -le 5 ]; then
  ...
  utils/combine_data.sh data/train_clean data/train_data
#  utils/combine_data.sh data/test_clean data/test_data_01
...
if [ $stage -le 6 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj $nj --cmd "$train_cmd" \
  	data/train_clean data/lang_nosp exp/mono
  if $decode; then
...
if [ $stage -le 7 ]; then
  ...
  steps/align_si.sh --boost-silence 1.25 --nj $nj --cmd "$train_cmd" \
  	data/train_clean data/lang_nosp exp/mono exp/mono_ali_5k
  # train a first delta + delta-delta triphone system on a subset of 5000 utterances
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" \
      2000 10000 data/train_clean data/lang_nosp exp/mono_ali_5k exp/tri1
  if $decode; then
...
if [ $stage -le 8 ]; then
  ...
  steps/align_si.sh --nj $nj --cmd "$train_cmd" \
    data/train_clean data/lang_nosp exp/tri1 exp/tri1_ali_10k
  # train an LDA+MLLT system.
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
     --splice-opts "--left-context=3 --right-context=3" 2500 15000 \
     data/train_clean data/lang_nosp exp/tri1_ali_10k exp/tri2
  if $decode; then
